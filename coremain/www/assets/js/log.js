                ...
                if (profile.type === 'switch9') {
                    (async () => {
                        ...
                        const results = await Promise.allSettled(cacheTags.map(cacheTag =>
                            api.fetch(`/plugins/${encodeURIComponent(cacheTag)}/flush`, { method: 'POST' })));
